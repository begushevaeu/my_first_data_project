...
- Офицеры по комплаенсу (оценка правомерности, отчётность)
- Технические специалисты (интеграторы) — DevOps/ML-инженеры, обеспечивают встраивание в инфраструктуру банка/финтеха

**Основные пользователи:**

- Команда кибербезопасности, фрод-аналитики, офицеры по комплайнсу (используют систему для выявления, проверки и подтверждения подозрительных транзакций и связей)
- Специалисты колл-центра (при блокировке транзакции клиент звонит в банк для уточнения ситуации и причин блокировки)
...
**Подцели:**
- Выявление сложных и ранее неуловимых схем отмывания денег с минимизацией ложных срабатываний
- Уменьшение false positives → сокращение расходов из-за некорректных срабатываний системы алертов
- Обнаружение сложных схем отмывания денег за пределами простых паттернов
...
**Текущие подходы:**
- Рулевые системы (rule-based)
- Классические ML-модели

**Недостатки:**
- Легко обходятся, много ложных срабатываний
- Не учитывают связи между аккаунтами, устройствами, IP и др.
...
2. **Датасеты:**  
   ...
   - Elliptic2 — граф транзакций в блокчейне Bitcoin для масштабных экспериментов
...
5. **Эксперты со стороны клиента:**  
   Желательно выделить antifraud-аналитика и технического специалиста для пилотного внедрения.
...
| Недостаточный уровень explainability | Внедрение GNNExplainer, attention-механизмов, визуализация подграфов, генерация интерпретируемых отчётов |
...
- Precision и Recall — при дисбалансе классов
- F1-score — общий баланс точности и полноты
- Дополнительно — PR-AUC
...
| **6. Финальный отчёт и презентация (Deployment)** | 19–27 мая | Подготовка документации, слайдов, отчёта |
| **7. Разработка MVP и защита проекта** | 28 мая – 4 июня | Разработка MVP, тестирование, финальная защита |
